...
public class Joueur {
    ...
    public Collection<Route> routesPourCompleterDestination(Destination d) {
        ...
        List<Route> routesDispo = new ArrayList<>(routes);
        routesDispo.addAll(getJeu().getRoutesLibres());
        HashMap<String, Ville> stringToVille = new HashMap<>();
        Collection<Route> routesChemin = new ArrayList<>();

        // Récupérer les objets Ville pour pouvoir appliquer l'algo des classes Graphes
        for(Route route : routesDispo) {
            for(String n : d.getVilles()) {
                if(route.getVille1().nom().equals(n)) {
                    stringToVille.put(n, route.getVille1());
                }
                if(route.getVille2().nom().equals(n)) {
                    stringToVille.put(n, route.getVille2());
                }
            }
        }

        // Réalisation du parcours en lui-même
        List<Integer> listeVilles = new ArrayList<>();
        if(d.getVilles().size() == 2) {
            // Destination simple
            listeVilles = Plateau.getGrapheStatic(routesDispo).parcoursSansRepetition(
                    stringToVille.get(d.getVilles().get(0)).getId(),
                    stringToVille.get(d.getVilles().get(1)).getId(),
                    true
            );
        }
        else {
            List<Integer> listeVillesATraverser = new ArrayList<>();
            for(int i = 0; i < d.getVilles().size(); i++) {
                listeVillesATraverser.add(stringToVille.get(d.getVilles().get(0)).getId());
            }
            listeVilles = Plateau.getGrapheStatic(routesDispo).parcoursSansRepetition(listeVillesATraverser);
        }

        if (listeVilles.isEmpty())
            return new ArrayList<>();
        // On reprend les villes qui composent le parcours
        for(int i = 0; i < listeVilles.size()-1; i++) {
            for(Route route : routesDispo) {
                if((route.getVille1().getId() == listeVilles.get(i) && route.getVille2().getId() == listeVilles.get(i+1))
                || (route.getVille2().getId() == listeVilles.get(i) && route.getVille1().getId() == listeVilles.get(i+1))) {
                    routesChemin.add(route);
                    break;
                }
            }
        }
    ...
    public Collection<Route> routesEnSurbrillancePourDestination(Destination d) {
        return routesPourCompleterDestinationAvecPions(d);
    }
}
