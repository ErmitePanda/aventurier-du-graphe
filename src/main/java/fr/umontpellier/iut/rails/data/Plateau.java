...
public class Plateau {
    ...
    /**
     * Construit le graphe de routes à partir des données du plateau.
    ...
    public Graphe getGraphe() {
        ...
        for(Route route : routes) {
            listeAretes.add(new Arete(route.getVille1().getId(), route.getVille2().getId(), route));
        }
        return new Graphe(listeAretes);
    }

    /**
     * Construit le graphe de routes à partir d'un ensemble de routes passé en paramètres.
     * Les sommets du graphe correspondent aux identifiants des villes (qui sont des entiers).
     */
    ...
    public static Graphe getGrapheStatic(Collection<Route> ensembleDeRoutes) {
        List<Arete> listeAretes = new ArrayList<>();
        for(Route route : ensembleDeRoutes) {
            listeAretes.add(new Arete(route.getVille1().getId(), route.getVille2().getId(), route));
        }
